...
void LayoutState::processAnimation(Director *director)
{
	...
	for (int i = 0; i < numPhotos; ++i) {
		...
		if (i == selectIndex) {
			...
			cv::rectangle(picture, cv::Rect(0,0,picture.getFrame().width,10), CV_RGB(255,0,0), CV_FILLED);
			cv::rectangle(picture, cv::Rect(0,0,10,picture.getFrame().height), CV_RGB(255,0,0), CV_FILLED);
			cv::rectangle(picture, cv::Rect(picture.getFrame().width-10,0,picture.getFrame().width,picture.getFrame().height), CV_RGB(255,0,0), CV_FILLED);
			cv::rectangle(picture, cv::Rect(0,picture.getFrame().height-10,picture.getFrame().width,picture.getFrame().height), CV_RGB(255,0,0), CV_FILLED);
		} else
